...
When(/^the tax is calculated$/) do
	...
	@tax_details = [["Social Security Contributions", "Income Tax"], [ssc.to_i.to_s, income_tax.to_i.to_s]]
end
...
Then(/^the following taxes are paid by the employee$/) do |expected_table|
  expected_table.diff!(@tax_details)
end
